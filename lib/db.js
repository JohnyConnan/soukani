...
export function loadDb(contentDir, { srcDir, fallbackDir = null }) {
  ...
  const db = {
    ...
    archivePages,
    previousWithPhotos,
    /** Past editions that link to the old site (the archive list). */
    externalEditions: editions.filter((e) => e.externalArchiveUrl),
    /** Editions whose photos live on the old site. Editions with nothing to show carry no URL. */
    photoEditions: editions.filter((e) => e.externalPhotosUrl),
  };
  return db;
}
