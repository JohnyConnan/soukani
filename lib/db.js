/**
 * Builds the `db` object from validated content. Pure given a directory; memoised per directory.
 */
...
export const STATES = ["announced", "open", "closed", "groupsKnown", "programmeOut", "running", "past", "archived"];

/**
 * @param {string} contentDir  directory holding the YAML files (fixtures may hold a subset)
 * @param {{ srcDir: string, fallbackDir?: string }} options
 */
export function loadDb(contentDir, { srcDir, fallbackDir = null }) {
  const key = `${contentDir}|${fallbackDir}`;
  if (cache.has(key)) return cache.get(key);

  const raw = loadContent(contentDir, fallbackDir);
  const problems = validate(raw, { srcDir });
  ...
  const hasRecords = (year) => Object.keys(collections).some((c) => forEdition(c, year).length > 0);
