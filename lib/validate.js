...
const PARTNER_TIERS = ["funding", "partner"]; // the two footer headings: "Finančně podpořili" and "Partneři"
...
export function validate(raw, { srcDir, assetDirs = [srcDir] }) {
  /** Referenced files are relative to src/ (fixtures may add their own asset directories). */
  const findAsset = (rel) => assetDirs.map((d) => join(d, rel)).find((p) => existsSync(p)) ?? null;
  const isSrcRelative = (v) => isNonEmptyString(v) && !v.startsWith("/") && !v.split("/").includes("..");
  const problems = [];
  ...
  function checkFile(file, id, field, value, { optional = true } = {}) {
    ...
    if (!isSrcRelative(value)) { add(file, id, field, `"${value}" must be a path relative to src/ without a leading slash (e.g. assets/photos/2027/01.jpg)`); return; }
    if (!findAsset(value)) add(file, id, field, `file not found: src/${value}`);
  }
  ...
  for (const key of ["home", "about", "applications", "groups", "programme", "workshops", "photos", "press", "archive"]) {
  ...
  for (const ed of editions) {
    ...
    // Participation conditions are printed together: set all six or none.
    const conditionFields = ["ageMin", "ageMax", "maxMinutes", "maxActors", "adultsMin", "adultsMax"];
    const conditionsSet = conditionFields.filter((f) => ed?.[f] != null);
    if (conditionsSet.length > 0 && conditionsSet.length < conditionFields.length) {
      for (const f of conditionFields) if (ed?.[f] == null) add("editions", id, f, `missing; set all of ${conditionFields.join(", ")} together, or none of them`);
    }
    for (const f of [...conditionFields, "logoYear"]) checkInt("editions", id, f, ed?.[f]);
    // Editions rendered by this site (the current one and locally archived ones) need an accent and all logo variants.
    const rendersLocally = ed?.year === raw.site?.currentEdition || (ed?.status === "archived" && !ed?.externalArchiveUrl);
    if (ed?.accent == null) {
      if (rendersLocally) add("editions", id, "accent", 'missing; editions shown on the site need a hex colour, written quoted like accent: "#FF6B2C"');
    } else if (!/^#[0-9A-Fa-f]{6}$/.test(ed.accent)) add("editions", id, "accent", `"${ed.accent}" must be a hex colour like #FF6B2C`);
    if (ed?.logo != null && typeof ed.logo !== "object") add("editions", id, "logo", "must be a map of variants: color, white, negative");
    else if (ed?.logo != null || rendersLocally) {
      for (const variant of ["color", "white", "negative"]) checkFile("editions", id, `logo.${variant}`, ed?.logo?.[variant], { optional: !rendersLocally });
      for (const variant of Object.keys(ed?.logo ?? {})) if (!["color", "white", "negative"].includes(variant)) add("editions", id, `logo.${variant}`, "unknown variant; use color, white and negative");
    }
    ...
    checkUrl("editions", id, "externalPhotosUrl", ed?.externalPhotosUrl);
  ...
  for (const f of ["shortName", "street", "postalCode", "town", "countryCode", "ico"]) if (!isNonEmptyString(String(site.organizer?.[f] ?? ""))) add("site", null, `organizer.${f}`, "missing");
  ...
  for (const f of ["name", "street", "postalCode", "town", "countryCode"]) if (!isNonEmptyString(String(site.venue?.[f] ?? ""))) add("site", null, `venue.${f}`, "missing");
  ...
  for (const p of raw.partners ?? []) {
    ...
    if (!PARTNER_TIERS.includes(p?.tier)) add("partners", id, "tier", `must be one of ${PARTNER_TIERS.join(", ")}`);
  }
  ...
  for (const p of photos) {
    ...
    if (!isSrcRelative(p?.file)) { add("photos", id, "file", `"${p?.file}" must be a path relative to src/ without a leading slash (e.g. assets/photos/2027/01.jpg)`); continue; }
    ...
    const full = findAsset(p.file);
    if (!full) add("photos", id, "file", `file not found: src/${p.file}`);
    else {
